...
複数のエージェントを連携させる代表的な3パターン（Workflow / Graph / Swarm）と、パターン内で状態を共有する `invocation_state` のサンプルです。`01`〜`03` は同じ題材（調査 → 分析 → まとめ）を3通りで実装して違いを比較でき、`04` はステート共有を別題材で示します。詳細は [`multiagent/README.md`](multiagent/README.md) を参照してください。
...
| `04_invocation_state.py` | 【ステートの共有】`invocation_state` で渡した使用言語・会社名を全ノード・全ツールで共有。書籍情報の提示（`book_manager → discount_manager`）を題材に、書籍名を渡すだけで言語別の書籍情報（定価含む）＋会社別の割引率が得られることを示す |
